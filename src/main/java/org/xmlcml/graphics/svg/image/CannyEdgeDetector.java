...
public class CannyEdgeDetector {
	...
	public void process() {
		...
		try {
			...
			throw new RuntimeException("Possibly due to png conversion???", e);
		}
	 ...
	 * 
	 * @param img
	 * @param lowThreshold ?0.5
	 * @param highThreshold ? 1.0
	 * @return
	 */
	public BufferedImage transformBufferedImage(BufferedImage img, float lowThreshold, float highThreshold) {
		 //adjust its parameters as desired
		 ...
		 this.process();
		 return this.getEdgesImage();
	}

	public static void main(String[] args) throws Exception {
		if (args.length != 1) {
			throw new RuntimeException("nned fileroot as arg");
		}
		String root = args[0];
		 ...
		 detector.process();
		 BufferedImage edges = detector.getEdgesImage();
		 ImageIO.write(edges, "png", new File(root+".png"));
	}
